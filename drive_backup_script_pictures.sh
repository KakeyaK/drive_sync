...
log_file="/home/kim/.drive_sync/pictures_log.txt"
max_lines=1000
...
if [ "$current_wifi_ssid" == "$desired_wifi_ssid" ]; then
    ...
    # Running the sync
    rclone bisync ~/Pictures/Drive 'gdrive:[3] Pictures' --remove-empty-dirs --resync --drive-export-formats link.html --check-access >> $log_file 2>&1
    result=$?    
